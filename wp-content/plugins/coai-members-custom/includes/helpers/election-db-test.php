...
if (!function_exists('coai_election_table')) {
    function coai_election_table($name = '') {
        global $wpdb;
        $map = [
            'elections'   => $wpdb->prefix . 'coai_elections',
            'positions'   => $wpdb->prefix . 'coai_election_positions',
            'candidates'  => $wpdb->prefix . 'coai_election_candidates',
            'votes'       => $wpdb->prefix . 'coai_election_votes',
            'vote_items'  => $wpdb->prefix . 'coai_election_vote_items',
        ];
        return $map[$name] ?? '';
    }
}
...
if (!function_exists('coai_get_open_election')) {
    function coai_get_open_election() {
        ...
        error_log('[COAI ELECTION] open-election db=' . DB_NAME);
        error_log('[COAI ELECTION] open-election prefix=' . $wpdb->prefix);
        error_log('[COAI ELECTION] open-election table=' . $table);
        error_log('[COAI ELECTION] open-election now=' . $now);

        $sql = $wpdb->prepare(
            "SELECT *
             FROM {$table}
             WHERE status = 'open'
               AND (opens_at IS NULL OR opens_at <= %s)
               AND (closes_at IS NULL OR closes_at >= %s)
             ORDER BY id DESC
             LIMIT 1",
            $now,
            $now
        ...
        error_log('[COAI ELECTION] open-election SQL=' . $sql);

        $row = $wpdb->get_row($sql, ARRAY_A);

        error_log('[COAI ELECTION] open-election last_error=' . $wpdb->last_error);
        error_log('[COAI ELECTION] open-election row-empty=' . (empty($row) ? 'YES' : 'NO'));

        if (!empty($row)) {
            error_log('[COAI ELECTION] open-election row-id=' . (int)($row['id'] ?? 0));
            error_log('[COAI ELECTION] open-election row-title=' . (string)($row['title'] ?? ''));
            error_log('[COAI ELECTION] open-election row-status=' . (string)($row['status'] ?? ''));
        }
